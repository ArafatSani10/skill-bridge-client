...
export default function Testimonials({ reviews }: { reviews: any[] }) {
  return (
    <section className="py-24 border-t border-zinc-100 dark:border-zinc-800 overflow-hidden">
        ...
        >
          {reviews?.map((review) => (
            <SwiperSlide key={review._id || review.id} className="!h-auto">
              <div className="flex flex-col h-full p-5 rounded-sm border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-transparent transition-all hover:border-zinc-300 dark:hover:border-zinc-700 shadow-sm">
                
                <div className="flex justify-between items-start mb-6">
                  <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-zinc-50 dark:bg-zinc-800 border border-zinc-100 dark:border-zinc-700">
                    <Star className="size-3 text-amber-500 fill-amber-500" />
                    <span className="text-[11px] font-bold text-zinc-700 dark:text-zinc-300">
                      {(review.rating || 5).toFixed(1)}
                    </span>
                  </div>
                  <Quote className="size-8 text-[#00baff]/20 fill-[#00baff]/10 rotate-180" strokeWidth={2} />
                </div>
